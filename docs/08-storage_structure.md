...
- raw — 1회 download (CPU pod 으로 cloud 에서 받음, 또는 host 에서 scp)
...
검증됨. 같은 region 의 여러 pod 가 동시 mount + read OK. write path 분리 시 contention 작음.
...
[host processed/] [container disk processed/]
        ...
        | (검증) | (rclone copy)
        v v
[3060 smoke train] [pCloud processed/] ← source of truth
                                                          |
                                                          | (학습 시 rclone copy)
                                                          v
...
```
# pod 안에서 (image 에 rclone v1.74 포함, 단 RunPod cache issue 로 v1.58 가능 — )
...
- **RunPod image 의 rclone version mismatch** — image build 의 v1.74 와 pod 의 v1.58 다를 수 있음. apt 의 v1.58 도 pCloud OAuth 호환 OK.
...
| Waymo GCS → CPU pod | 110 MB/s | parallel |
